...
module.exports = async(req,res,next)=>{
    ...
    if(!(entID && entMatricule)) res.end(JSON.stringify({error:true,message:"Missing parameters"}))
        ...
        if(authed.type == "entreprise"){
            ...
            else res.end(JSON.stringify({error:true,message:"Cannot access this enterprise as another"}))
        }
        else res.end(JSON.stringify({error:true,message:"Permission denied"}))
    }
}